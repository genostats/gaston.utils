#include <Rcpp.h>
#include <iostream>
#ifndef GASTON_OPTIM3
#define GASTON_OPTIM3
...
class fun_optim {
  ...
  public:
    ...
    bool newton_min(scalar_t & x, const scalar_t min_x, const scalar_t max_x, const scalar_t eps, int max_iter, const bool verbose, const bool brent_fallback);
    void newton_min(scalar_t & x, const scalar_t min_x, const scalar_t max_x, const scalar_t eps, int max_iter, const bool verbose);

    bool newton_max(scalar_t & x, const scalar_t min_x, const scalar_t max_x, const scalar_t eps, int max_iter, const bool verbose, const bool brent_fallback) {
      scale = -1;
      bool converged = newton_min(x, min_x, max_x, eps, max_iter, verbose, brent_fallback);
      scale = 1;
      return converged;
    }
...
// returns true if the algorithm converged (always true if brent_fallback is true)
template<typename scalar_t>
bool fun_optim<scalar_t>::newton_min(scalar_t & x, const scalar_t min_x, const scalar_t max_x, const scalar_t eps, int max_iter, const bool verbose, const bool brent_fallback) {
  ...
  while(std::abs(df) > 2*eps) {
    i++;
    if(i > max_iter) { // NOTE si max_iter = 0 on n'itère pas du tout, 
      if(!brent_fallback) return false;
...
template<typename scalar_t>
void fun_optim<scalar_t>::newton_min(scalar_t & x, const scalar_t min_x, const scalar_t max_x, const scalar_t eps, int max_iter, const bool verbose) {
  bool converged = fun_optim<scalar_t>::newton_min(x, min_x, max_x, eps, max_iter, verbose, true);
}

// ceci piqué dans le code de R 
// pour utiliser optimize() depuis le code C !
template<typename scalar_t>
scalar_t fun_optim<scalar_t>::Brent_fmin(scalar_t ax, scalar_t bx, scalar_t tol) {
